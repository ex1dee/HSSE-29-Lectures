...
Тогда $a^ \frac{p-1}{2}v \equiv_{p} n$, но $vn \equiv_{p} (p-1)! \equiv_{p}-1$, т.к. $vn$ - просто произведение всех остатков по модулю $p \implies a^ \frac{p-1}{2} v^2 \equiv_{p} -1$ 

$НОД(v_{i}, p) = 1 \implies \ \exists v_{i}^{-1} : v_{i}v_{i}^{-1} \equiv_{p} 1 \implies \left( \frac{v_{i}v_{i}^{-1}}{p} \right) = \left( \frac{v_{i}}{p} \right) * \left( \frac{v_{i}^{-1}}{p} \right) =$
$=1 * \left( \frac{v_{i}^{-1}}{p} \right) =\left( \frac{1}{p} \right) = 1 \implies (\frac{v_{i}^{-1}}{p})=1$

Причем нет пересекающихся пар вычетов ($v_{i}, v_{i}^{-1}$), т.к. квадратичные вычеты между собой различны, и каждому соответствует единственный обратный квадратичный вычет.

Значит, произведение $v^2$ содержит в себе все такие пары ($v_{i}, v_{i}^{-1}$) $\implies$ $v^2\equiv_{p}1 \implies a^ \frac{p-1}{2} \equiv_{p}-1$, ч.т.д.