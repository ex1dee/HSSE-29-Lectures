...
=> по теореме Вейерштрасса имеет предел, равный её инфимуму => $\\lim_{ n \to \infty } y_{n} = inf y_{n}$ => перейдём к предельному неравенству и получим, что $\overline{\lim}_{ n \to \infty }x_{n} \leq infy_{n}$
