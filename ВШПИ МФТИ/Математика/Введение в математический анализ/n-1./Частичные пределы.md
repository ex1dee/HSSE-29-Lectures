...
4. Установим неравенство $\overline{\lim}_{ n \to \infty } x_{n} \geq inf y_{n} = inf sup_{k\geq n} x_{k}$. Достаточно доказать, что правая часть $inf y_{n} = inf sup_{k\geq n} x_{k}$ является частичным пределом $\{ x_{n} \}$.
